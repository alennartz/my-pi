...
The scoped extension factory supplies identity and communication explicitly rather than through process environment variables. Every child runtime gets fresh cwd-bound settings, resources, event bus, tools, skills, trust state, and extension instances; only root auth storage and the model registry are shared. Existing child JSONL sessions and version-1 lifecycle logs remain the persistence boundary, while the live registry is intentionally not persisted. The subprocess, RPC, and Unix-socket transports are removed rather than kept as a fallback.
