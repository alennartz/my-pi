...
- `extensions/subagents/group.ts` — group lifecycle, RPC child management, state tracking (per-turn input, context window resolution, subgroup detection, waitingFor targets with correlation-to-target mapping), idle detection, parent message callback delegation
