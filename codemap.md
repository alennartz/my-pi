...
- `extensions/subagents/group.ts` — group lifecycle, RPC child management, fork-aware spawning (branches on `AgentSpec.kind`), shared temp session directory lifecycle (mkdtemp on start, rmSync on destroy), state tracking (per-turn input, context window resolution, subgroup detection, waitingFor targets with correlation-to-target mapping), idle detection, parent message callback delegation
